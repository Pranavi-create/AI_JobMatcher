...
```
┌─────────────────────────────────────────────────────────────┐
│  1. JOB COLLECTION (Fresh Data - Timestamped)               │
│  ───────────────────────────────────────────                │
│  • LinkedIn Collector → linkedin_collector/data/            │
│    - linkedin_scraper.py runs                               │
│    - Searches LinkedIn API + The Muse API                   │
│    - Saves: linkedin_jobs_YYYYMMDD_HHMMSS.json             │
│                                                             │
│  • GitHub Collector → github_collector/data/                │
│    - github_fetcher.py runs                                 │
│    - Fetches from SimplifyJobs, etc.                        │
│    - Saves: github_jobs_YYYYMMDD_HHMMSS.json               │
│                                                             │
│  • Firecrawl Collector → API_collector/data/ (Optional)     │
│    - collect_firecrawl_jobs.py runs                         │
│    - LLM-powered web scraping                               │
│    - Saves: firecrawl_jobs_YYYYMMDD_HHMMSS.json            │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│  2. AI JOB MATCHING (job_matcher.py)                        │
│  ─────────────────────────────────                          │
│  • Load ONLY most recent file from each collector           │
│    (Avoids duplicates - old files kept as backup)           │
│  • Extract resume from resume/ folder                       │
│  • Gemini 2.5 Flash AI analyzes each job                    │
│  • Calculate match scores (0-100)                           │
│  • Generate detailed match reasoning                        │
│  • Rank and select top 50 matches                           │
│  • Save to: matched_jobs/top_50_matches.json                │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│  3. EMAIL DELIVERY (send_email_smtp.py)                     │
│  ────────────────────────────────────                       │
│  • Load top 50 matches from JSON                            │
│  • Format email with:                                       │
│    - Job title, company, location                           │
│    - Match score and reasoning                              │
│    - Apply link                                             │
│  • Send via Gmail SMTP                                      │
│  • ✅ Check your inbox for matched jobs!                    │
└─────────────────────────────────────────────────────────────┘

Alternative: Use Claude Desktop MCP (job_matcher_mcp_complete.py)
  → All steps run automatically via natural language commands!
```
